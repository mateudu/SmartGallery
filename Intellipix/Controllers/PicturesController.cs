...
namespace SmartGallery.Web.Controllers
{
    ...
    public class PicturesController : Controller
    {
        public PicturesController(CloudStorageAccount account, CommentRepository context)
        {
            _account = account;
            _context = context;
            TagMinConfidence = double.Parse(ConfigurationManager.AppSettings["vision:tagminconfidence"],
                CultureInfo.InvariantCulture);
        }

        private readonly CommentRepository _context;
        private readonly CloudStorageAccount _account;
        private readonly double TagMinConfidence;
        ...
        public async Task<ActionResult> Upload(HttpPostedFileBase file)
        {
            if (file != null && file.ContentLength > 0)
            {
                ...
                else
                {
                    ...
                    var tags = result.Tags.Where(x => x.Confidence >= TagMinConfidence).ToList();

                    for (int i = 0; i < tags.Count; i++)
                    {
                        string key = $"Tag{i}";
                        photo.Metadata.Add(key, tags[i].Name);
                    }
